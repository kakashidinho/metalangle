...
#define kThreadGroupXYZ                                                          \
    (kGenerate3DMipThreadGroupSizePerDim * kGenerate3DMipThreadGroupSizePerDim * \
     kGenerate3DMipThreadGroupSizePerDim)

#define kThreadGroupXY (kGenerate3DMipThreadGroupSizePerDim * kGenerate3DMipThreadGroupSizePerDim)
#define kThreadGroupX kGenerate3DMipThreadGroupSizePerDim
...
#define OUT_OF_BOUND_CHECK(edgeValue, targetValue, condition) \
    ...
    if ((lIndex & 0x49) == 0)  // (lIndex & b1001001) == 0
    {
        ...
        // (x+1, y+1, z)
        float4 texel5 =
            OUT_OF_BOUND_CHECK(texel2, TEXEL_LOAD(lIndex + (kThreadGroupX + 1)), atEdge.y);
        // (x+1, y, z+1)
        float4 texel6 =
            OUT_OF_BOUND_CHECK(texel2, TEXEL_LOAD(lIndex + (kThreadGroupXY + 1)), atEdge.z);
        // (x, y+1, z+1)
        float4 texel7 = OUT_OF_BOUND_CHECK(
            texel3, TEXEL_LOAD(lIndex + (kThreadGroupXY + kThreadGroupX)), atEdge.z);
        // (x+1, y+1, z+1)
        float4 texel8 = OUT_OF_BOUND_CHECK(
            texel5, TEXEL_LOAD(lIndex + (kThreadGroupXY + kThreadGroupX + 1)), atEdge.z);
    ...
    if ((lIndex & 0xdb) == 0)  // (lIndex & b11011011) == 0
    {
        ...
        // (x+1, y+1, z)
        float4 texel5 =
            OUT_OF_BOUND_CHECK(texel2, TEXEL_LOAD(lIndex + (2 * kThreadGroupX + 2)), atEdge.y);
        // (x+1, y, z+1)
        float4 texel6 =
            OUT_OF_BOUND_CHECK(texel2, TEXEL_LOAD(lIndex + (2 * kThreadGroupXY + 2)), atEdge.z);
        // (x, y+1, z+1)
        float4 texel7 = OUT_OF_BOUND_CHECK(
            texel3, TEXEL_LOAD(lIndex + (2 * kThreadGroupXY + 2 * kThreadGroupX)), atEdge.z);
        // (x+1, y+1, z+1)
        float4 texel8 = OUT_OF_BOUND_CHECK(
            texel5, TEXEL_LOAD(lIndex + (2 * kThreadGroupXY + 2 * kThreadGroupX + 2)), atEdge.z);
    ...
    if ((lIndex & 0x1ff) == 0)  // (lIndex & b111111111) == 0
    {
        ...
        // (x+1, y+1, z)
        float4 texel5 =
            OUT_OF_BOUND_CHECK(texel2, TEXEL_LOAD(lIndex + (4 * kThreadGroupX + 4)), atEdge.y);
        // (x+1, y, z+1)
        float4 texel6 =
            OUT_OF_BOUND_CHECK(texel2, TEXEL_LOAD(lIndex + (4 * kThreadGroupXY + 4)), atEdge.z);
        // (x, y+1, z+1)
        float4 texel7 = OUT_OF_BOUND_CHECK(
            texel3, TEXEL_LOAD(lIndex + (4 * kThreadGroupXY + 4 * kThreadGroupX)), atEdge.z);
        // (x+1, y+1, z+1)
        float4 texel8 = OUT_OF_BOUND_CHECK(
            texel5, TEXEL_LOAD(lIndex + (4 * kThreadGroupXY + 4 * kThreadGroupX + 4)), atEdge.z);
