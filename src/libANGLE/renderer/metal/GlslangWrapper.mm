...
uint32_t GenerateTransformFeedbackOutputs(const gl::ProgramState &programState,
                                          IntermediateShaderSource *vertexShader)
{
    // TODO(hqle): only ES 2.0 is supported atm
    return kXfbBindingIndexStart;
}
                            ...
                            IntermediateShaderSource *fragmentSource)
{
    ...
    // See corresponding code in OutputVulkanGLSL.cpp.
    std::string vertexDefaultUniformsBinding   = "binding = " + Str(kDefaultUniformsBindingIndex);
    std::string fragmentDefaultUniformsBinding = "binding = " + Str(kDefaultUniformsBindingIndex);
    ...
    const std::string driverBlockLayoutString = "binding = " + Str(kDriverUniformsBindingIndex);
                            ...
                            IntermediateShaderSource *fragmentSource)
{
    ...
    for (unsigned int uniformIndex : programState.getSamplerUniformRange())
    {
        ...
        const std::string setBindingString      = "binding = " + Str(textureBinding);
                                        ...
                                        std::string *fragmentSourceOut)
{
    ...
    uint32_t lastLayoutBoundIndex = kDriverUniformsBindingIndex;
...
// static
angle::Result GlslangWrapperMtl::GetShaderCodeImpl(mtl::ErrorHandler *context,
                                                   const gl::Caps &glCaps,
                                                   const std::string &vertexSource,
                                                   const std::string &fragmentSource,
                                                   std::vector<uint32_t> *vertexCodeOut,
